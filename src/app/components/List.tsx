...
const List = (props: Props): React.ReactElement => {
  const { entries, options, pageSize, generateSelectInputItems, handleOnSelect } = props
  ...
  useEffect(() => {
    if (listBuffer.length == 0) {
      const entryItems: Item[] = entries.map((entry: ListEntry) => {
        ...
        const text = `[${idx}] [${extension}] ${title}`
    ...
    >
      <Box width="100%" flexDirection="column">
        {renderList.map((item: Item, i: number) => {
          ...
          const itemInDownloadQueue: boolean = item.data
            ? downloadQueue.indexOf(item.data) > -1
            : false
          const selectInputItems: SelectInputItem[] = generateSelectInputItems(
            itemChecked,
            itemInDownloadQueue
          )
            ...
            if (item.expandable && item.data) {
              return (
                <ExpandableListItem
                  ...
                  onSelect={(returned: returnedValue) =>
                    handleOnSelect(expanded, setExpanded, item.data, returned)
                  }
                >
                  {item.text}
                </ExpandableListItem>
            ...
            if (!item.expandable && item.value) {
              return (
                <ListItem
                  ...
                  fadedOut={itemFadedOut}
                  onSelect={(returned: returnedValue) =>
                    handleOnSelect(expanded, setExpanded, null, returned)
                  }
                >
                  {item.text}
                </ListItem>
