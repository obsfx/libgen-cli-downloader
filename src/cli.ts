...
cl.option('-b, --bulk <MD5LIST.txt>', 'start the app in bulk downloading mode')
...
const bulk = async (filename: string) => {
  try {
    ...
    const setInitialStatus: (initialStatus: AppStatus) => void =
      useStore.getState().set.initialStatus
...
const url = async (md5: string) => {
  ...
  const downloadMirror: string | null = await findDownloadMirror(
    mirror,
    configJSON.searchByMD5Pattern,
    md5,
    onErr,
    error_tolarance,
    error_reconnect_delay_ms
