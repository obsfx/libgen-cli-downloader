...
const sleep = (ms: number): Promise<void> =>
  new Promise(
    (resolve: (value?: undefined) => void): NodeJS.Timeout => setTimeout(() => resolve(), ms)
  )
...
const getDocument = async (URL: string): Promise<HTMLDocument | null> => {
  const response: Response | null = await getResponse(URL)
...
export const doRequest = (
  ...
  new Promise(async (resolve: (param: Response | null) => void) => {
    let errCount = 0
...
export const isPageExist = async (
  searchReqPattern: string,
  mirror: string,
  query: string,
  pageNumber: number,
