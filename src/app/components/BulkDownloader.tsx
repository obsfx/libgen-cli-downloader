...
const BulkDownloader = (props: Props): React.ReactElement => {
  const { mode } = props
  const { exit } = useApp()
  ...
  const setLastFailedAction: (lastFailedAction: () => void) => void = useStore(
    ...
    <Box flexDirection="column">
      <Box>
        ...
        {state.status == 'onGoing' && (
          <Text>
            ...
            <Text color="greenBright">
              {state.queue.filter((item: QueueItem) => item.status == 'completed').length} /{' '}
              {state.queue.length}
            </Text>
            <Text> to </Text>
        ...
        {state.status == 'completed' && (
          <Text>
            <Text color="greenBright">
              {state.queue.filter((item: QueueItem) => item.status == 'completed').length} of{' '}
              {state.queue.length}
            </Text>
      ...
      <Box flexDirection="column">
        {(state.status == 'onGoing' || state.status == 'completed') &&
          state.queue.map((item: QueueItem, i: number) => (
            <Box key={i} flexDirection="column">
              {item.errorCounter > 0 &&
                (item.status == 'processing' || item.status == 'downloading') && (
                  <Text>
                    <Text color="red">
                      {item.errorCounter} / {error_tolarance}{' '}
                    </Text>
              ...
              <Text wrap="truncate">
                ...
                {item.status == 'downloading' && (
                  <Text>
                    <Text color="greenBright">
                      {' '}
                      {((100 / item.total) * item.progress).toFixed(2)}%
                    </Text>
                    &nbsp;
                    <Text color="magentaBright">
                      {pretty(item.progress)}/{pretty(item.total)}
                    </Text>
                    &nbsp;
