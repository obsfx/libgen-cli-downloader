...
export const startDownloading = (
  ...
  onEnd: (filename: string) => void
): Promise<true | null> =>
  new Promise(async (resolve: (param: true | null) => void) => {
    const downloadResponse: Response | null = await doRequest(
      endpoint,
      onErr,
      errorTolarance,
      delay
    ...
    if (Buffer.byteLength(parsedContentDisposition.parameters.filename, 'utf8') > 255) {
      ...
      parsedContentDisposition.parameters.filename =
        parsedContentDisposition.parameters.filename.substring(contLen - 100, contLen)
    }

    const path = `./${parsedContentDisposition.parameters.filename}`
    const file: fs.WriteStream = fs.createWriteStream(path)

    const total = Number(downloadResponse?.headers.get('content-length') || 0)
...
export const findDownloadMirror = async (
  ...
  const reqURL: string = searchByMD5Pattern.replace('{mirror}', mirror).replace('{md5}', md5)
...
export const findMD5s = async (
  ...
  const reqURL: string = MD5ReqPattern.replace('{mirror}', mirror).replace('{id}', idStr)
