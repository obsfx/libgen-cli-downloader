...
const SelectInput = (props: Props) => {
  const { selectInputItems, onSelect, hideInfo } = props

  const [cursorPos, setCursorPos] = useState(0)
  const [input, setInput] = useState(false)

  const items: SelectInputItem[] = selectInputItems.filter(
    (option: SelectInputItem) => !option.disabled
  )
    ...
    <Box flexDirection="column" width="100%">
      <Box flexDirection="column" paddingLeft={2} width="100%">
        {items.map((option: SelectInputItem, i: number) => (
          <SelectInputItem key={i} hovered={cursorPos == i}>
            {option.label}
          </SelectInputItem>
        ))}
      </Box>
