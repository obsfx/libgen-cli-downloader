...
const Results = (): React.ReactElement => {
  ...
  const filters: [string, string][] = Object.entries(
    useStore((state) => state.globals.searchFilters)
  ).filter(([_, value]) => value.trim() != '')

  let entries: ListEntry[] = useStore((state) => state.globals.entries).map(
  ...
  const setDownloadQueue: (callback: (entries: Entry[]) => Entry[]) => void = useStore(
    (state) => state.set.downloadQueue
  )
  ...
  const generateSelectInputItems = (
    inBulkQueue: boolean,
    inDownloadQueue: boolean
  ): SelectInputItem[] => [
    {
    ...
    {
      label: !inDownloadQueue ? (
        <Text>Dowload Directly</Text>
      ) : (
        <Text>
    ...
    {
      label: !inBulkQueue ? (
        <Text>Add To Bulk Downloading Queue</Text>
      ) : (
        <Text>Remove From Bulk Downloading Queue</Text>
      ),
      value: !inBulkQueue
  ...
  const handleOnSelect = (
    ...
    switch (returned) {
      ...
      case returnedValue.downloadDirectly:
        if (entryData) {
          setDownloadQueue((entries: Entry[]) => [...entries, entryData])
        }
        break
    ...
    <Box flexDirection="column">
      ...
      {bulkWarning && (
        <Text color="yellow" bold={true}>
          Bulk Downloading Queue is Empty
        </Text>
      )}
      <BulkQueueIndicator />
      <Text wrap="truncate">
