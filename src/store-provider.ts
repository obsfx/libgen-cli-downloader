...
type Setters = {
  ...
  lastFailedAction: (lastFailedAction: () => void) => void
  ...
  downloadQueue: (callback: (entries: Entry[]) => Entry[]) => void
  query: (query: string) => void
  searchFilters: (
    callback: (entries: Globals) => {
      author: string
      publisher: string
      year: string
      pages: string
...
export const useStore = create<State>(
  (set: SetState<State>): State => ({
    ...
    set: {
      ...
      initialStatus: (initialStatus: AppStatus) =>
        set((state) => ({ globals: { ...state.globals, initialStatus } })),
      nextPage: (nextPage: boolean) =>
        set((state) => ({ globals: { ...state.globals, nextPage } })),
      lastFailedAction: (lastFailedAction: () => void) =>
        set((state) => ({ globals: { ...state.globals, lastFailedAction } })),
      errorCounter: (errorCounter: number) =>
        set((state) => ({ globals: { ...state.globals, errorCounter } })),
      currentPage: (currentPage: number) =>
        set((state) => ({ globals: { ...state.globals, currentPage } })),
      mirror: (mirror: string) => set((state) => ({ globals: { ...state.globals, mirror } })),
      entries: (entries: Entry[]) => set((state) => ({ globals: { ...state.globals, entries } })),
      entryBuffer: (entryBuffer: Entry) =>
        set((state) => ({ globals: { ...state.globals, entryBuffer } })),
      listBuffer: (listBuffer: Item[]) =>
        set((state) => ({ globals: { ...state.globals, listBuffer } })),
      bulkQueue: (bulkQueue: string[]) =>
        set((state) => ({ globals: { ...state.globals, bulkQueue } })),
      downloadQueue: (callback: (entries: Entry[]) => Entry[]) =>
        set((state) => ({
          globals: { ...state.globals, downloadQueue: callback(state.globals.downloadQueue) },
        })),
      ...
      searchFilters: (
        ...
        set((state) => ({ globals: { ...state.globals, searchFilters: callback(state.globals) } })),
    },
  })
)
